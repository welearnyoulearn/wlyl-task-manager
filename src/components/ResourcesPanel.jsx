...
import { FolderOpen, Link2, Paperclip } from 'lucide-react';
...
// Team-wide reference board, maintained by admins: dev/staging links,
// credentials, shared drive/folder links, docs - anything that isn't a
// task but the whole team needs to find in one place. Visible to every
// signed-in member regardless of role; only admins can add, edit, or
// delete an entry (see supabase/019_resources.sql for the RLS backing
// this - the same select-open/write-admin-only pattern as every other
// team-wide table in this app).
export default function ResourcesPanel({ active }) {
  const { currentUser, currentUserId, isAdmin } = useAuth();
  const { resources, loadResources } = useData();
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (editingKey) {
        const { error } = await sb.from('resources').update({ ...payload, updated_at: new Date().toISOString() }).eq('id', editingKey);
        ...
        if (error) throw error;
        toast({ description: `"${payload.title}" added to Resources.` });
      }
  ...
  const deleteResource = async (resource) => {
    try {
      const { error } = await sb.from('resources').delete().eq('id', resource.key);
      if (error) throw error;
      await loadResources();
  ...
  return (
    <div className={`panel ${active ? 'active' : ''}`} id="panel-resources">
      <div className="sheet" style={{ maxWidth: 760, margin: '0 auto' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 4 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            ...
            <div>
              <div className="section-title" style={{ marginBottom: 4 }}>Resources</div>
              <div className="section-hint">Dev/staging links, credentials, shared folders, docs — anything the whole team needs to find in one place.</div>
            </div>
          </div>
          {isAdmin && <Button size="sm" onClick={openAddForm}>Add Resource</Button>}
        </div>

        <div style={{ marginTop: 16 }}>
          {resources.length === 0 && (
            <div className="section-hint">No resources yet.{isAdmin ? ' Click "Add Resource" to add the first one.' : ''}</div>
          )}
          {resources.map(r => (
      ...
      <Dialog open={showForm} onOpenChange={(open) => { if (!open) setShowForm(false); }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingKey ? 'Edit resource' : 'Add a resource'}</DialogTitle>
            <DialogDescription>Visible to every signed-in member and admin.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-3">
            <div>
              ...
              <Input
                type="text"
                placeholder="e.g. Staging environment access"
                value={draft.title}
